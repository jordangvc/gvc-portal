...
## 🤖 5 Daily COACH — the portal's FIRST LLM feature (r113) — BUILT 2026-08-09
...
chasing ghosts. Tests: `tests/test_nonneg_coach.py` (13). Hub **r113**. (r110–r112 were taken mid-flight by the concurrent session's #178 — this work was stash-recovered from dangling commit 77b6b558.)
...
parallel (~4s). Brief first paint skips Open Drive; `/ui/api/morning/gfolders`
fills links after render. Ports Morning half of conflicting #141. Hub **r92**.
...
`money-scope` — verbal adds, standing-by, T&M discipline. Also shipped
**touchup-drywall** on the same wave. Hub **r83**.
...
finalize carry Next links. Coach catalog import at module top; Field Guide
shell↔catalog wiring regression-tested. Perf-audit documents Monday `_query`
retries. Hub **r78**.
...
catalog related/next into related. hang/scrape steps re-synced from HTML.
Catalog now also owns Job Check gates **qc-walk** (Notes) + **closeout-rhythm**
(Completion Date) — no shell-only exemptions left in
`JOBCHECK_FIELDGUIDE_ANCHORS`. OSS reuse map: `docs/OSS-REUSE-MAP.md`. Hub **r76**.
...
when Vision counts >1 checks (server fail-safe; UI soft-stop is separate).
...
Invoice stays on the spine (office daily click-through). Hub **r67**.
...
longer waits on a cold list). Billing + Estimate also POST `/ui/api/monday/warm`
so Path jumps that skip the hub still hydrate L1/L2 Monday caches. Hub **r65**.
