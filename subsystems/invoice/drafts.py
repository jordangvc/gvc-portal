"""
Invoice draft store — shared, resumable in-progress invoices, backed by a single
JSON object in GCS.
=========================================================================
The exact sibling of estimate_drafts.py, for the invoice form. The invoice form
is long (client + job + line items + commercial/AIA fields), and a session can
time out or a connection drop mid-fill. The browser autosaves to localStorage so
nothing is lost locally; this module is the SHARED server copy so any teammate
with the `invoice` grant can resume or delete a draft from any device.

NOT a source of truth — a draft is pre-submission scratch. The invoice only
becomes real at the live run (Stripe + the Monday ledger row). Drafts are deleted
on a successful live create.

The pure logic (validation, upsert/remove/cap, last-writer-wins) is shared with
estimate_drafts; only the GCS object differs (portal/invoice-drafts.json) so the
two never collide. Reuses portal_store's bucket/creds plumbing.
"""
...
DEFAULT_OBJECT = "portal/invoice-drafts.json"


def _object_name() -> str:
    return os.environ.get("GVC_INVOICE_DRAFTS_OBJECT") or DEFAULT_OBJECT
...
# ---------------------------------------------------------------------------
# Public API (used by the /ui/api/invoice/drafts routes)
# ---------------------------------------------------------------------------
