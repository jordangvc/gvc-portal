...
Slice 2 (this file): long-term-hold detection (updated_at), the 14-workday
update-author relevance rule, posting a Monday update (photo-upload flow),
and resolving a linked project's GFolder Link. Action Requests board, routes,
and Fireflies are still later slices — see docs/MORNING_BRIEF_BUILD_SPEC.md.
...
def _people(cv: dict) -> list[dict]:
    ...
    Teams/agents are skipped for Ops. Owner matching.
    """
    entities = [p for p in (cv.get("persons_and_teams") or []) if p]
    raw_text = (cv.get("text") or _column_text(cv) or "").strip()
...
def get_gfolder_url_for_ops_item(mc, item_id: int) -> Optional[str]:
    ...
    Returns a Drive folder URL or None.
    """
    col = boards.MORNING_COL_PROJECT_LINK
    gcol = boards.MORNING_PROJECTS_GFOLDER_COL
    q1 = """
    ...
    """
    try:
        data = mc._query(q1, {"ids": [str(int(item_id))], "cols": [col]})
